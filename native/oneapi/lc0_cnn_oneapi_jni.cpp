/*
 * native/oneapi/lc0_cnn_oneapi_jni.cpp
 *
 * Overview
 * --------
 * This translation unit implements the optional oneAPI backend for the Java LC0 evaluator in this repo
 * (see src/chess/nn/lc0/cnn/oneapi/Support.java and src/chess/nn/lc0/cnn/oneapi/Backend.java).
 *
 * It builds into a shared library (lc0_oneapi) that is loaded via JNI and provides:
 ...
 *   - chess.nn.lc0.cnn.oneapi.Support.nativeDeviceCount() -> int
 *   - chess.nn.lc0.cnn.oneapi.Backend.nativeCreate(String weightsPath) -> long (opaque handle)
 *   - chess.nn.lc0.cnn.oneapi.Backend.nativeDestroy(long handle) -> void
 *   - chess.nn.lc0.cnn.oneapi.Backend.nativeGetInfo(long handle) -> long[7]
 *   - chess.nn.lc0.cnn.oneapi.Backend.nativePredict(long handle, float[] encoded, float[] policyOut, float[] wdlOut) -> float
 *
 * Data / shapes
 * -------------
...
extern "C" JNIEXPORT jint JNICALL Java_chess_nn_lc0_cnn_oneapi_Support_nativeDeviceCount(JNIEnv*, jclass) {
    return device_count();
}
...
extern "C" JNIEXPORT jlong JNICALL Java_chess_nn_lc0_cnn_oneapi_Backend_nativeCreate(JNIEnv* env, jclass, jstring jpath) {
    return backend_nativeCreate(env, jpath);
}

extern "C" JNIEXPORT void JNICALL Java_chess_nn_lc0_cnn_oneapi_Backend_nativeDestroy(JNIEnv*, jclass, jlong handle) {
    destroy_net(reinterpret_cast<SyclNet*>(handle));
}

extern "C" JNIEXPORT jlongArray JNICALL Java_chess_nn_lc0_cnn_oneapi_Backend_nativeGetInfo(JNIEnv* env, jclass, jlong handle) {
...
extern "C" JNIEXPORT jfloat JNICALL Java_chess_nn_lc0_cnn_oneapi_Backend_nativePredict(
