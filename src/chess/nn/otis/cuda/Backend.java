...
/**
 * Optional CUDA backend (JNI) for OTIS policy/WDL inference.
 *
 * <p>This uses a native shared library ({@code otis_cuda}) and will only be used when:
 * <ul>
 ...
 * {@code -Dcrtk.otis.backend=auto} and CUDA is available (current system properties only).
 *
...
public final class Backend implements AutoCloseable {
    ...
    /**
     * Creates a CUDA evaluator from a ChessRTK OTIS {@code .bin} weights file.
     *
    ...
    public static Backend create(Path weightsBin) {
        NativeBackendOps.Created created = NativeBackendOps.create(
                ...
                "Failed to create CUDA OTIS evaluator (no device / init failed).",
                "CUDA OTIS evaluator returned invalid info.");
