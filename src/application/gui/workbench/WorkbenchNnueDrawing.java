...
/**
 * Stateless tooltip, vector, and trace drawing helpers for {@link WorkbenchNnueView}.
 */
final class WorkbenchNnueDrawing {
     ...
     */
    static void paintHeaderChip(Graphics2D g, Rectangle r,
     ...
     */
    static String stockfishStackSummary(float[] transformed, float[] transformedUs,
     ...
     */
    static void highlightSquare(java.awt.Graphics2D g, Rectangle board, int square,
     ...
     */
    static float[] subtractVectors(float[] a, float[] b) {
     ...
     */
    static float[] withoutLast(float[] values) {
     ...
     * @return stats label
     */
    static String traceStats(float[] values, boolean signed) {
        if (values == null || values.length == 0) {
            return "no data";
        }
     ...
     */
    static int safeLength(float[] values) {
     ...
     */
    static int adaptivePitch(int usable, int count, int nominal) {
        if (count <= 1) {
            return 0;
        }
     ...
     */
    static void drawStageHeader(Graphics2D g, int cx, int y, int width,
     ...
     */
    static void drawInactiveFeatureLane(Graphics2D g, int cx, int y, int radius) {
     ...
     */
    static void drawTraceEdge(Graphics2D g, int x1, int y1, int x2, int y2,
     ...
     */
    static void drawCenteredFittedLabel(Graphics2D g, String text, int cx, int y, int maxWidth) {
        FontMetrics fm = g.getFontMetrics();
     ...
     * @return fitted text
     */
    static String fitText(FontMetrics fm, String text, int maxWidth) {
     ...
     * @return index or -1
     */
    static int strongestAbsIndex(float[] values) {
        if (values == null || values.length == 0) {
            return -1;
        }
        int best = 0;
     ...
     */
    static void drawGridCellRing(Graphics2D g, Rectangle grid, int index,
     ...
     * @return scale
     */
    static float matrixScale(float[] values) {
     ...
     */
    static float visiblePairScale(float[] us, float[] them, int[] slots) {
        float scale = 0.0f;
        if (slots != null) {
            for (int idx : slots) {
     ...
     * @return non-zero scale
     */
    static float visibleAbsScale(float[] values, int[] slots) {
        float scale = 0.0f;
        if (slots != null) {
            for (int idx : slots) {
    ...
    private static float maxAbs(float[] data) {
        return WorkbenchNetworkView.maxAbs(data);
    }
}
