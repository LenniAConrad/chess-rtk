...
public final class PublishPreview extends JComponent {
    ...
    private void paintDocument(Graphics2D g) {
        ...
        g.setColor(Theme.withAlpha(Theme.MUTED, 32));
    ...
    private void paintCover(Graphics2D g, int x, int y, int w, int h) {
        ...
        drawElided(g, preview.title(), Theme.font(20, Font.BOLD), Theme.TEXT,
                x + w / 10, y + h / 3, w - w / 5);
        if (!preview.subtitle().isBlank()) {
            drawElided(g, preview.subtitle(), Theme.font(12, Font.PLAIN), Theme.MUTED,
    ...
    private void paintDiagramPage(Graphics2D g, int x, int y, int w, int h) {
        drawElided(g, preview.title(), Theme.font(14, Font.BOLD), Theme.TEXT, x, y + 12, w);
        ...
        if (!preview.noFen()) {
            ...
            drawElided(g, preview.source(), Theme.mono(9), Theme.MUTED,
                    x, sourceY, w);
            bodyTop = sourceY + gap;
        }
    ...
    private void paintBookPage(Graphics2D g, int x, int y, int w, int h) {
        drawElided(g, preview.title(), Theme.font(13, Font.BOLD), Theme.TEXT, x, y + 10, w);
        drawRule(g, x, y + 22, w);
    ...
    private void paintFooter(Graphics2D g, int x, int y, int w) {
        ...
        g.setColor(preview.ready() ? Theme.STATUS_SUCCESS_TEXT : Theme.STATUS_WARNING_TEXT);
