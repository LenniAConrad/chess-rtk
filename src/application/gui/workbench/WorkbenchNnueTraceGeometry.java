...
/**
 * Trace geometry helpers for {@link WorkbenchNnueView}.
 */
final class WorkbenchNnueTraceGeometry {
     ...
     * Utility class.
     */
    private WorkbenchNnueTraceGeometry() {
    }
     ...
     */
    static int[] traceColumnCenters(WorkbenchNnueTraceLayout layout) {
     ...
     */
    static int traceBandWidth(WorkbenchNnueTraceLayout layout) {
     ...
     */
    static Color traceStageTint(int index) {
        switch (index) {
            case 0:
     ...
     * @return center y
     */
    static int columnY(WorkbenchNnueTraceLayout layout, int row, int count) {
        if (count <= 1) {
            return (layout.graphTop + layout.graphBottom) / 2;
        }
        int pitch = WorkbenchNnueDrawing.adaptivePitch(layout.usableHeight, count, TRACE_SLOT_PITCH);
