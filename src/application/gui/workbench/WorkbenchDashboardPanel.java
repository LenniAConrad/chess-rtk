 ...
 *
 * <p>The panel renders entirely from the shared {@link WorkbenchSession} model
 * (plus its {@link WorkbenchJobManager} and {@link WorkbenchArtifactIndex}); it
 * never scrapes Swing components or console text. Quick-action buttons delegate
 * to the host window through {@link WorkbenchDashboardActions}.</p>
 */
final class WorkbenchDashboardPanel extends JPanel implements WorkbenchSessionListener {
     ...
     */
    private final transient WorkbenchSession session;

    /**
     * Action delegate routing quick actions back to the host window.
     */
    private final transient WorkbenchDashboardActions actions;
     ...
     */
    private final transient WorkbenchJobTableModel jobModel;
     ...
     */
    WorkbenchDashboardPanel(WorkbenchSession session, WorkbenchDashboardActions actions) {
        super(new BorderLayout());
        this.session = session;
        this.actions = actions;
        this.jobModel = new WorkbenchJobTableModel(session.jobs());
        ...
        leftColumn.add(Box.createVerticalStrut(WorkbenchTheme.SPACE_SM));
        leftColumn.add(buildBatchCard());
        leftColumn.add(Box.createVerticalGlue());

        JPanel rightColumn = stackColumn();
        rightColumn.add(buildEngineCard());
        rightColumn.add(Box.createVerticalStrut(WorkbenchTheme.SPACE_SM));
        rightColumn.add(buildHealthCard());
        rightColumn.add(Box.createVerticalGlue());

        JPanel topColumns = WorkbenchUi.transparentPanel(
                new GridLayout(1, 2, WorkbenchTheme.SPACE_SM, 0));
        topColumns.add(leftColumn);
        topColumns.add(rightColumn);

        JPanel grid = WorkbenchUi.transparentPanel(new GridBagLayout());
        grid.setBorder(WorkbenchTheme.pad(WorkbenchTheme.SPACE_MD));
        ...
        c.insets = new Insets(WorkbenchTheme.SPACE_XS, WorkbenchTheme.SPACE_XS,
                WorkbenchTheme.SPACE_XS, WorkbenchTheme.SPACE_XS);
        ...
        grid.add(WorkbenchUi.transparentPanel(null), c);
        ...
        JPanel centered = WorkbenchUi.transparentPanel(new GridBagLayout());
        ...
        add(WorkbenchUi.scroll(centered), BorderLayout.CENTER);
    ...
    private JComponent buildJobsCard() {
        ...
        jobTable.setRowHeight(WorkbenchTheme.CONTROL_HEIGHT - 4);
        jobTable.getTableHeader().setReorderingAllowed(false);
        jobTable.setAutoResizeMode(JTable.AUTO_RESIZE_SUBSEQUENT_COLUMNS);
        WorkbenchUi.styleComponentTree(jobTable);
        // Give each column a sensible share so the command and result columns
        // get the room and status/duration stay compact.
        jobTable.getColumnModel().getColumn(WorkbenchJobTableModel.COL_COMMAND)
                .setPreferredWidth(320);
        jobTable.getColumnModel().getColumn(WorkbenchJobTableModel.COL_STATUS)
                .setPreferredWidth(90);
        jobTable.getColumnModel().getColumn(WorkbenchJobTableModel.COL_DURATION)
     ...
     */
    @Override
    public void sessionChanged(WorkbenchSession changed) {
        render();
    }
     ...
     */
    private void withSelectedJob(java.util.function.Consumer<WorkbenchJob> consumer) {
    ...
    private static JLabel caption(String text) {
        ...
        label.setFont(WorkbenchTheme.font(10, Font.PLAIN));
    ...
    private static JComponent infoRow(String caption, JLabel value) {
        JPanel row = WorkbenchUi.transparentPanel(new BorderLayout(WorkbenchTheme.SPACE_SM, 0));
        row.setAlignmentX(Component.LEFT_ALIGNMENT);
        JLabel captionLabel = new JLabel(caption);
        captionLabel.setFont(WorkbenchTheme.font(12, Font.BOLD));
        captionLabel.setForeground(WorkbenchTheme.MUTED);
        captionLabel.setPreferredSize(new Dimension(112, WorkbenchTheme.CONTROL_HEIGHT - 8));
        captionLabel.setVerticalAlignment(SwingConstants.TOP);
        row.add(captionLabel, BorderLayout.WEST);
        row.add(value, BorderLayout.CENTER);
        row.setBorder(WorkbenchTheme.pad(WorkbenchTheme.SPACE_XS / 2, 0));
        return row;
    }
    ...
    private static JComponent actionRow(JButton... buttons) {
        JPanel row = WorkbenchUi.transparentPanel(
                new java.awt.FlowLayout(java.awt.FlowLayout.LEFT, WorkbenchTheme.SPACE_SM, 0));
        row.setAlignmentX(Component.LEFT_ALIGNMENT);
    ...
    private static JButton quickButton(String text, Runnable action) {
        return WorkbenchUi.button(text, false, event -> action.run());
    }
    ...
    private static Color colorForPhase(float value) {
        if (value < 0.25f) {
            return WorkbenchTheme.STATUS_INFO_BORDER;
        }
        if (value < 0.68f) {
            return WorkbenchTheme.ACCENT;
        }
        return WorkbenchTheme.STATUS_WARNING_BORDER;
    }
    ...
    private static Color colorForMobility(float value) {
        ...
        if (value > 0.75f) {
            return WorkbenchTheme.STATUS_SUCCESS_BORDER;
        }
        return WorkbenchTheme.ACCENT;
    }
    ...
    private static Color colorForSafety(float value) {
        if (value < 0.36f) {
            return WorkbenchTheme.STATUS_ERROR_BORDER;
        }
        if (value < 0.66f) {
            return WorkbenchTheme.STATUS_WARNING_BORDER;
        }
    ...
    private static JPanel cardBody() {
        JPanel body = WorkbenchUi.transparentPanel(null);
    ...
    private static JPanel stackColumn() {
        JPanel column = WorkbenchUi.transparentPanel(null);
    ...
    private static final class MaterialStrip extends JComponent {
        ...
        protected void paintComponent(Graphics graphics) {
            ...
            try {
                ...
                g.drawString(WorkbenchUi.elide(detail, detailMetrics, Math.max(0, w - 20)),
                        10, h - 10);
                ...
                g.setColor(WorkbenchTheme.withAlpha(WorkbenchTheme.STATUS_WARNING_BORDER, 190));
    ...
    private static final class MetricMeter extends JComponent {
        ...
        void setValue(float newValue, String newLabel, Color color) {
            ...
            accent = color == null ? WorkbenchTheme.ACCENT : color;
            repaint();
        }
        ...
        protected void paintComponent(Graphics graphics) {
            ...
            try {
                ...
                String visible = WorkbenchUi.elide(label, fm, Math.max(0, w - 20));
                ...
                g.setColor(WorkbenchTheme.withAlpha(accent, 210));
            ...
            try {
                ...
                for (String tag : tags) {
                    ...
                    Color fill = shown % 3 == 0
                            ? WorkbenchTheme.STATUS_INFO_BG
                            : WorkbenchTheme.ELEVATED_SOLID;
                    g.setColor(fill);
                    g.fillRoundRect(x, y, chipW, 17, 8, 8);
                    g.setColor(WorkbenchTheme.LINE);
                    g.drawRoundRect(x, y, chipW, 17, 8, 8);
                    g.setColor(WorkbenchTheme.TEXT);
                    g.drawString(WorkbenchUi.elide(visible, fm, chipW - 10), x + 8, y + 12);
                    x += chipW + 5;
                    shown++;
                }
                if (shown < tags.size()) {
    ...
    private static final class HealthRings extends JComponent {
         ...
         */
        void setChecks(WorkbenchHealthSnapshot.Check configCheck,
                WorkbenchHealthSnapshot.Check doctorCheck,
                WorkbenchHealthSnapshot.Check smokeCheck) {
            config = configCheck == null ? WorkbenchHealthSnapshot.Check.UNKNOWN : configCheck;
            doctor = doctorCheck == null ? WorkbenchHealthSnapshot.Check.UNKNOWN : doctorCheck;
            smoke = smokeCheck == null ? WorkbenchHealthSnapshot.Check.UNKNOWN : smokeCheck;
            repaint();
        }
        ...
        private static void paintRing(Graphics2D g, int x, int y, String label,
                WorkbenchHealthSnapshot.Check check) {
            int d = 34;
         ...
         * @return colour
         */
        private static Color checkColor(WorkbenchHealthSnapshot.Check check) {
            return switch (check) {
                case OK -> WorkbenchTheme.STATUS_SUCCESS_BORDER;
                case FAILED -> WorkbenchTheme.STATUS_ERROR_BORDER;
                case RUNNING -> WorkbenchTheme.STATUS_WARNING_BORDER;
                case UNKNOWN -> WorkbenchTheme.STATUS_INFO_BORDER;
            };
        }
         ...
         * @return glyph
         */
        private static String checkGlyph(WorkbenchHealthSnapshot.Check check) {
            return switch (check) {
                case OK -> "OK";
                case FAILED -> "!";
                case RUNNING -> "...";
