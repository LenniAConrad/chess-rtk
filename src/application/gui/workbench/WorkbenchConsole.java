 ...
 * </ul>
 */
final class WorkbenchConsole extends JTextPane {
    ...
    WorkbenchConsole() {
        setEditable(false);
        setOpaque(true);
        setBackground(WorkbenchTheme.TERMINAL);
        setForeground(WorkbenchTheme.TERMINAL_TEXT);
        setCaretColor(WorkbenchTheme.TERMINAL_TEXT);
        setSelectionColor(WorkbenchTheme.TEXT_SELECTION);
        setSelectedTextColor(WorkbenchTheme.TERMINAL_TEXT);
        setFont(WorkbenchTheme.mono(13));
        setBorder(WorkbenchTheme.pad(7, 9, 7, 9));
    }
    ...
    private SimpleAttributeSet styleFor(String line) {
        ...
        if (trimmed.startsWith("[exit ") || trimmed.startsWith("[error")) {
            return style(WorkbenchTheme.STATUS_ERROR_TEXT, true);
        }
        if (trimmed.startsWith("[stopped]") || trimmed.startsWith("[")) {
            return style(WorkbenchTheme.STATUS_WARNING_TEXT, true);
        }
        if (lower.contains("error") || lower.contains("exception")
                || lower.contains("failed") || lower.contains("invalid")) {
            return style(WorkbenchTheme.STATUS_ERROR_TEXT, false);
        }
        return style(WorkbenchTheme.TERMINAL_TEXT, false);
    }
