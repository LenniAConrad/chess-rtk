 ...
 */
final class WorkbenchTomlHighlighter {
     ...
     */
    private static final Color SECTION = WorkbenchTheme.STATUS_INFO_TEXT;

    /**
     * String literal text color.
     */
    private static final Color STRING = WorkbenchTheme.STATUS_SUCCESS_TEXT;

    /**
     * Number literal text color.
     */
    private static final Color NUMBER = WorkbenchTheme.STATUS_ERROR_TEXT;

    /**
     * Boolean/null literal text color.
     */
    private static final Color BOOLEAN = WorkbenchTheme.STATUS_WARNING_TEXT;

    /**
     * TOML comment text color.
     */
    private static final Color COMMENT = WorkbenchTheme.MUTED;

    /**
     * Prevents instantiation.
     */
    private WorkbenchTomlHighlighter() {
        // utility
    }
     ...
     */
    static void apply(JTextPane pane, String text) {
        String source = text == null ? "" : text;
        StyledDocument doc = pane.getStyledDocument();
        try {
            doc.remove(0, doc.getLength());
            doc.insertString(0, source, attrs(WorkbenchTheme.TEXT, false, false));
            if (!source.isEmpty()) {
                doc.setCharacterAttributes(0, source.length(),
                        attrs(WorkbenchTheme.TEXT, false, false), true);
            }
