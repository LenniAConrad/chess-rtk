...
public final class SvgIcon implements Icon {
     ...
     */
    public SvgIcon(Kind kind, int size, Color color) {
        this.kind = kind;
        this.size = size;
        this.color = color;
    }
    ...
    private static Kind kindForLabel(String text) {
        ...
        Kind kind = switch (text) {
            ...
            case "Publish", "Browse", "Load File", "Save PGN", "Save Report", "Choose Input", "Choose Output",
                    "Choose PDF", "Choose Cover", "Choose Manifest", "Choose Protocol" ->
                    Kind.FILE;
            default -> null;
        };
        if (kind == null) {
    ...
    private void paintLogo(Graphics2D g) {
        g.setColor(Theme.LOGO_BACKGROUND);
        g.fillRoundRect(1, 1, 22, 22, 5, 5);
        g.setColor(Theme.LOGO_MARK);
