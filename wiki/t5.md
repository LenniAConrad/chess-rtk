...
Use `tag-text` when you want a short summary for one FEN or a FEN file:

```bash
crtk tag-text --fen "<FEN>" --model models/t5.bin
crtk tag-text -i seeds.txt --model models/t5.bin --include-fen
```
