...
- `chess.engine` contains the alpha-beta search behind the default classical/NNUE `engine builtin` path and Workbench Play, plus the MCTS search used by LC0/OTIS and the minimal UCI loop. `chess.eval` is the in-process evaluator facade for `engine eval` and `engine static`.
