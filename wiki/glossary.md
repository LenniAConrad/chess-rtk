...
| **MCTS** | Monte Carlo Tree Search. The search algorithm behind crtk's built-in engine, which grows a search tree by sampling promising lines under a playout budget. Invoked via `engine builtin` (alias `engine java`). |
